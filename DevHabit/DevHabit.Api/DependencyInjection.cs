...
public static class DependencyInjection
{
    public static WebApplicationBuilder AddApiServices(this WebApplicationBuilder builder)
    {
        ...
        {
            options.ReturnHttpNotAcceptable = true; // To return this response when an Accept header is not supported
        })
            .AddNewtonsoftJson(options => options.SerializerSettings.ContractResolver =
                new CamelCasePropertyNamesContractResolver())
            .AddXmlSerializerFormatters();

        // Adding support for some custom media types.
            ...
            options
                .UseNpgsql(
                    ...
                    npgsqlOptions => npgsqlOptions
                        .MigrationsHistoryTable(HistoryRepository.DefaultTableName, Schemas.Application)) // Allow us to customize the migrations history table name and schema
        ...
        builder.Services.AddDbContext<ApplicationIdentityDbContext>(options =>
            options
                .UseNpgsql(
                    builder.Configuration.GetConnectionString("Database"),
                    npgsqlOptions => npgsqlOptions
                        .MigrationsHistoryTable(HistoryRepository.DefaultTableName, Schemas.Identity)) // Allow us to customize the migrations history table name and schema
                .UseSnakeCaseNamingConvention());
    ...
    public static WebApplicationBuilder AddApplicationServices(this WebApplicationBuilder builder)
    {
        ...
        // To test the Polly Resilence Handler
                ...
                if (!string.IsNullOrEmpty(identityId))
                {
                    return RateLimitPartition.GetTokenBucketLimiter(
                        ...
                        _ =>
                            ...
                            {
                                TokenLimit = 100,
                                QueueProcessingOrder = QueueProcessingOrder.OldestFirst, // After the limit is reached, the next requests will be queued and processed in the order they were received (FIFO)
                                QueueLimit = 5,
                                ReplenishmentPeriod = TimeSpan.FromMinutes(1),
                                TokensPerPeriod = 25 // After a minute, 25 tokens will be added to the bucket, allowing the next 25 requests to be processed. If there are queued requests, they will be processed as tokens are replenished.
                            });
                }

                // If the user is not authenticated, we can apply a more strict policy to prevent abuse. In this case, we will allow only 5 requests per minute for anonymous users.
