    ...
    UserContext userContext) : ControllerBase
{
        ...
        IValidator<CreateEntryImportJobDto> validator)
    {
        ...
        var importJob = new EntryImportJob
        {
            ...
            FileContent = memoryStream.ToArray(), // Store this into a object storage like Azure Blob Storage or AWS S3 in production
            CreatedAtUtc = DateTime.UtcNow
        };

        // Storing it into the DB
        ...
        // Creating a unique job detail for each import job, so that we can track the progress and update the status of the import job in the database
        ...
        // Starting the Job right away, but you can also schedule it to run after some delay if needed
        ...
        // Crating the response DTO with the links if needed
        ...
        // Returning 201 Created with the location header pointing to the newly created import job
