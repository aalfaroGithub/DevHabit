...
//[DisableRateLimiting] // Can be used at Controller label or endpoint level to disable the global/default rate limiting policy
    ...
    UserContext userContext) : ControllerBase
{
        ...
        DataShapingService dataShapingService)
    {
        ...
        List<EntryDto> entries = await entriesQuery
            .OrderByDescending(e => e.Date) // Order by date desc to get the most recent entries first required for cursor pagination
            .ThenByDescending(e => e.Id) // Order by ID desc to ensure consistent ordering for entries with the same date, using Id as the cursor index
            .Take(query.Limit + 1)
            .Select(EntryQueries.ProjectToDto())
            .ToListAsync();
        ...
        // ExpandoObject is used for the Data Shaping service
