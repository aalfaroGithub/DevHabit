...
{
    // For optmistic locking, we only check for PUT and PATCH requests, as POST and DELETE typically don't require concurrency control in the same way.
    ...
    public async Task InvokeAsync(HttpContext context, InMemoryETagStore etagStore)
    {
        ...
        // Checking the headers if the client has sent an ETag for concurrency control
        ...
        // If is 200 ok response and content type is json
